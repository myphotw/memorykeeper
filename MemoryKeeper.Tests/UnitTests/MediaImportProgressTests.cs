...
public class MediaImportProgressTests
{
    [Fact]
    public async Task ImportAsync_ReportsProgressCounts()
    {
        var sourceRoot = Path.Combine(Path.GetTempPath(), $"mk-progress-src-{Guid.NewGuid():N}");
        var libraryRoot = Path.Combine(Path.GetTempPath(), $"mk-progress-lib-{Guid.NewGuid():N}");
        Directory.CreateDirectory(sourceRoot);
        Directory.CreateDirectory(libraryRoot);

        await File.WriteAllBytesAsync(Path.Combine(sourceRoot, "a.jpg"), "a"u8.ToArray());
        await File.WriteAllBytesAsync(Path.Combine(sourceRoot, "b.jpg"), "b"u8.ToArray());
        ...
        try
        {
            ...
            Assert.Contains(reports, report => report.TotalCount == 2 && report.ProcessedCount == 0);
            Assert.Contains(reports, report => report.IsCompleted && report.ImportedCount == 2);
            Assert.Equal(2, reports.Last(report => !report.IsCompleted).ProcessedCount);
        }
        finally
        {
    ...
    private sealed class CollectingProgress : IProgress<ImportProgressDto>
    {
        private readonly List<ImportProgressDto> _reports;

        public CollectingProgress(List<ImportProgressDto> reports)
        {
            _reports = reports;
        }

        public void Report(ImportProgressDto value) => _reports.Add(value);
    }
